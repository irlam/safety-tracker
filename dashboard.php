...
// Work out actual column names in your DB
$COL_TOUR_DATE = col_exists($pdo, 'safety_tours','tour_date')      ? 'tour_date'
               : (col_exists($pdo,'safety_tours','tour_datetime')  ? 'tour_datetime' : 'tour_date'); // default
$COL_STATUS    = col_exists($pdo, 'safety_tours','status')         ? 'status'
               : (col_exists($pdo,'safety_tours','tour_status')    ? 'tour_status' : 'status');
$HAS_PCT       = col_exists($pdo, 'safety_tours','score_percent');
$HAS_A         = col_exists($pdo, 'safety_tours','score_achieved');
$HAS_T         = col_exists($pdo, 'safety_tours','score_total');

// Build a safe expression for the % column
$scoreExpr = $HAS_PCT
  ? 'score_percent'
  : ($HAS_A && $HAS_T ? '(CASE WHEN score_total>0 THEN ROUND(score_achieved/score_total*100,2) ELSE NULL END)' : 'NULL');
...
if ($from !== '') { $where[] = "$COL_TOUR_DATE >= ?"; $args[] = date('Y-m-d 00:00:00', strtotime($from)); }
if ($to   !== '') { $where[] = "$COL_TOUR_DATE <= ?"; $args[] = date('Y-m-d 23:59:59', strtotime($to)); }
if ($q    !== '') { $where[] = '(site LIKE ? OR area LIKE ?)'; $args[] = "%$q%"; $args[] = "%$q%"; }
if ($status === 'Open' || $status === 'Closed') { $where[] = "$COL_STATUS = ?"; $args[] = $status; }
...
try {
  $stmt = $pdo->prepare("
    SELECT id,
           $COL_TOUR_DATE   AS tour_date,
           site, area, lead_name,
           $COL_STATUS      AS status,
           $scoreExpr       AS score_percent
    FROM safety_tours
    $whereSql
    ORDER BY id DESC
    LIMIT 500
...
// Show an error banner if the query failed (so you’re not flying blind)
if ($lastSqlError) {
  echo '<div style="margin:10px 0;padding:10px;border:1px solid #7f1d1d;background:#3b0a0a1a;border-radius:12px;color:#fecaca">
          <strong>SQL error:</strong> '.htmlspecialchars($lastSqlError).'
        </div>';
}

// Derive top-card stats
$totalTours = 0;
$openCount  = 0;
$closedCount= 0;
$avgScore   = null;
...
try {
  $sparkWhere = ["DATE($COL_TOUR_DATE) BETWEEN ? AND ?"];
  $sparkArgs  = [$start30, date('Y-m-d')];

  if ($q    !== '') { $sparkWhere[] = '(site LIKE ? OR area LIKE ?)'; $sparkArgs[] = "%$q%"; $sparkArgs[] = "%$q%"; }
  if ($status === 'Open' || $status === 'Closed') { $sparkWhere[] = "$COL_STATUS = ?"; $sparkArgs[] = $status; }

  $sparkSql = 'WHERE '.implode(' AND ', $sparkWhere);
  $stmt = $pdo->prepare("
    SELECT DATE($COL_TOUR_DATE) d, COUNT(*) c
    FROM safety_tours
    $sparkSql
    GROUP BY DATE($COL_TOUR_DATE)
  ");
  $stmt->execute($sparkArgs);
  foreach ($stmt->fetchAll() as $row) {
    $d = $row['d']; if (isset($countsByDay[$d])) $countsByDay[$d] = (int)$row['c'];
  }
} catch (Throwable $e) {
  // optional: echo the error for sparkline too
}
...
/* -----------------------------
   Actions stats (filtered, joined to tours)
   - $actsTotal: all actions (even if priority/severity is NULL)
   - $actsLow/$actsMed/$actsHigh: for the badges (normalized)
------------------------------*/
$actsTotal = 0;
...
if (table_exists($pdo, 'safety_actions')) {
  try {
    // Build WHERE with the same filters you use elsewhere
    $aw = []; $aa = [];

    // Date range -> filter by the tour's date/time column
    if ($from !== '') { $aw[] = "t.$COL_TOUR_DATE >= ?"; $aa[] = date('Y-m-d 00:00:00', strtotime($from)); }
    if ($to   !== '') { $aw[] = "t.$COL_TOUR_DATE <= ?"; $aa[] = date('Y-m-d 23:59:59', strtotime($to)); }

    // Site / Area -> filter on tours
    if ($q !== '') { $aw[] = '(t.site LIKE ? OR t.area LIKE ?)'; $aa[] = "%$q%"; $aa[] = "%$q%"; }

    // Status dropdown -> use ACTION status so the card reflects action counts
    if ($status === 'Open' || $status === 'Closed') { $aw[] = 'a.status = ?'; $aa[] = $status; }

    $awSql = $aw ? ('WHERE ' . implode(' AND ', $aw)) : '';

    // 1) Total actions (regardless of priority/severity)
    $stmt = $pdo->prepare("
      SELECT COUNT(*) AS c
    ...
    ");
    $stmt->execute($aa);
    $actsTotal = (int)$stmt->fetchColumn();

    // 2) Per-priority breakdown with normalization
    //    - Trim + lower
    //    - Map common variants to low/medium/high
    $stmt = $pdo->prepare("
      SELECT bucket, COUNT(*) AS c FROM (
        SELECT
          CASE
            WHEN LOWER(TRIM(COALESCE(a.priority, a.severity))) IN ('high','hi','urgent') THEN 'high'
            WHEN LOWER(TRIM(COALESCE(a.priority, a.severity))) IN ('medium','med','mid') THEN 'medium'
            WHEN LOWER(TRIM(COALESCE(a.priority, a.severity))) IN ('low','lo') THEN 'low'
            ELSE 'other'
          END AS bucket
        FROM safety_actions AS a
        LEFT JOIN safety_tours  AS t ON t.id = a.tour_id
    ...
    $actsLow = $actsMed = $actsHigh = 0; // reset in case of partial results
    foreach ($stmt->fetchAll() as $r) {
      switch ($r['bucket']) {
        case 'low':    $actsLow  = (int)$r['c']; break;
        case 'medium': $actsMed  = (int)$r['c']; break;
        case 'high':   $actsHigh = (int)$r['c']; break;
        // 'other' is ignored in badges but still part of $actsTotal
      }
    }
  } catch (Throwable $e) {
    // If you want to surface errors temporarily:
    // echo '<div class="err"><strong>Actions SQL:</strong> '.htmlspecialchars($e->getMessage()).'</div>';
  }
}
...
<style>
  ...
  *{box-sizing:border-box} html,body{margin:0;padding:0}
  body{background:radial-gradient(1200px 800px at 75% -100px, rgba(14,165,233,.10), transparent 60%),var(--bg);color:var(--text);font:16px/1.6 system-ui,-apple-system,Segoe UI,Roboto}
  ...
  .navbtn{padding:10px 16px;border-radius:14px;border:1px solid var(--border);background:linear-gradient(180deg, rgba(14,165,233,.25), rgba(2,132,199,.12));box-shadow:0 12px 24px rgba(2,132,199,.08) inset;color:#dbeafe;font-weight:800}
  .grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}
  @media (max-width:1100px){.grid{grid-template-columns:repeat(2,1fr)}}
  @media (max-width:620px){.grid{grid-template-columns:1fr}}
  .card{background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.015));border:1px solid var(--border);border-radius:var(--radius);padding:16px}
  ...
  .low{background:#064e3b;color:#d1fae5}.med{background:#3f2e00;color:#fde68a}.hi{background:#3b0a0a;color:#fecaca}
  ...
  .btn{background:linear-gradient(180deg, var(--accent1), var(--accent2));border:0;border-radius:14px;color:#00131a;font-weight:800;padding:12px 16px;cursor:pointer;box-shadow:0 20px 40px rgba(2,132,199,.25) inset}
  .btn-ghost{background:linear-gradient(180deg, rgba(255,255,255,.08), rgba(255,255,255,.03));color:#d1d5db;border:1px solid var(--border)}
...
<body>
  <div class="wrap">
    ...
    <?php if ($isAdmin && ($lastSqlError || $rowsTotalInTable !== null)): ?>
      <div class="err">
        <div><strong>Debug:</strong> <?= $lastSqlError ? htmlspecialchars($lastSqlError) : 'No SQL error.' ?></div>
        <div>Total rows in <code>safety_tours</code>: <?= (int)$rowsTotalInTable ?></div>
        <?php if ($whereSql): ?>
          <div>Applied filter: <code><?= htmlspecialchars($whereSql) ?></code></div>
        <?php endif; ?>
      </div>
    <?php endif; ?>
    ...
    <?php if ($showDeletedToast): ?>
      ...
      <script>
        (function(){var t=document.getElementById('toast');if(!t)return;requestAnimationFrame(function(){t.classList.add('show')});setTimeout(function(){t.classList.remove('show')},2600);})();
      </script>
    <?php endif; ?>
    ...
    <section class="grid">
      <div class="card">
        ...
        <div class="spark">
          <svg viewBox="0 0 <?= $sparkW ?> <?= $sparkH ?>" preserveAspectRatio="none" aria-hidden="true">
            <defs><linearGradient id="grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#38bdf8" stop-opacity="0.65"/><stop offset="100%" stop-color="#0ea5e9" stop-opacity="0.25"/></linearGradient></defs>
      ...
      <div class="card">
        ...
        <?php $total = max(1, $openCount + $closedCount); $openPct = round(($openCount/$total)*100); ?>
      ...
      <div class="card">
        ...
        <div class="big"><?= (int)$actsTotal ?></div>
        <div class="badges">
        ...
        <div style="margin-top:12px"><a class="btn" href="/actions.php">Open register →</a></div>
      </div>
    </section>
    ...
    <div class="card" style="margin-top:14px">
      <table>
        ...
        <?php else: foreach ($rows as $r): ?>
          <tr>
            <td><?= (int)$r['id'] ?></td>
            <td><?= h(date('d/m/Y H:i', strtotime($r['tour_date']))) ?></td>
            ...
            <?php if ($isAdmin): ?>
              <td class="right">
                <a href="/delete_tour.php?id=<?= (int)$r['id'] ?>&back=<?= urlencode('dashboard.php') ?>"
                   style="display:inline-block;padding:8px 10px;border-radius:10px;border:1px solid #7f1d1d;background:linear-gradient(180deg,#ef4444,#b91c1c);color:#fff">Delete</a>
              </td>
            <?php endif; ?>
          </tr>
