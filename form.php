...
// Load previously used recipients from the **correct** table
$known = [];
try {
  $st = db()->query("SELECT email, COALESCE(email,'') AS label, use_count FROM safety_recipient_emails ORDER BY use_count DESC, COALESCE(last_used, created_at) DESC LIMIT 50");
  $known = $st->fetchAll();
...
<div class="wrap">
  ...
  <form action="submit.php" method="post" enctype="multipart/form-data" class="card" id="tourForm">
    ...
    <?php endforeach; ?>
<?php
  // Fetch previously used recipients from the correct table name
  $known = [];
  try {
    $st = db()->query("
      SELECT email, COALESCE(NULLIF(label,''), email) AS label, use_count, last_used
      FROM safety_recipient_emails
      ORDER BY (last_used IS NOT NULL) DESC, last_used DESC, use_count DESC
      LIMIT 50
    ");
    $known = $st->fetchAll();
  } catch (Throwable $e) {
...
<div class="card">
  ...
  <!-- chips + input -->
  <div id="recChips" class="chips" style="margin-top:8px"></div>
  <input type="email" id="recInput" placeholder="name@company.com" autocomplete="off" spellcheck="false" inputmode="email">
...
<script>
// Wire the “Previously used” buttons to your chips
(function(){
  ...
  function addEmail(raw){
    ...
    set.add(email); input.value=''; paint();
  }
  ...
  document.querySelectorAll('.rec-suggest').forEach(btn=>{
    btn.addEventListener('click', ()=> addEmail(btn.dataset.email||''));
  });
})();
</script>
